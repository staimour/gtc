...
common hdata,hmenu,parthist,fieldhist,modehist
  ...
  modedesc='drift_m=10_n=0_antturnsoff'
...
;		printf,1,y(0:799,0:1)
     ...
     31: begin		;momentum
		y=parthist(*,2:3,0)
    print,"start=",y(0),"end=",y((n_elements(y)/2)-1)
...
; panel 1: mode history of real and imaginary components
	if smoothnum eq 0 then begin
	  yr=y(nstart:nend,0)
	  yi=y(nstart:nend,1)
    ;save, yr,yi,filename='/global/u1/c/clau/redo_m=10/913_edge_history.sav'
	endif else begin
	  yr=Smooth(y(nstart:nend,0),smoothnum);y(nstart:nend,0)
	  yi=Smooth(y(nstart:nend,1),smoothnum);y(nstart:nend,1)
	endelse
	xx=xtime(nstart:nend)
...
;	endfor
		i=i+1
	endwhile
	;save, modehist,filename='/global/u1/c/clau/redo_m=10/omni_m=10_950.sav'
	ioerr: close, ihistory
	ndstep=long(i)
	
	ntime=long(ndstep)
	xtime= indgen(ntime,/LONG)
	nstart=0
	nend=ntime-1
